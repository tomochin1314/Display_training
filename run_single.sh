...
function task1()
{
	...
	TECHS=( $(echo "phong phong_halo phong_des phong_des_halo tone tone_halo tone_des tone_des_halo hatching hatching_halo hatching_des hatching_des_halo" | sed -r 's/(.[^;]*;)/ \1 /g' | tr " " "\n" | shuf | tr -d " " ) )
	echo "focus order:" ${FBARRARY[*]} >> $LOG
	echo "tech order:" ${TECHS[*]} >> $LOG
	local di=0
	for NORMALCASEDATADIR in  ${ALLNORMALCASEDATADIRS}
	do
		for n in ${CURORDER}
...
if [ $# -lt 4 ];then
    echo "Usage: $0 <participant index> <rendering technique> <halo> <shadow> [resolution]"
    echo ""
    echo "          participant index:    an unsigned integer number."
    echo "          rendering technique: \"phong\" or  \"tone\" or  \"hatching\"."
    echo "          halo:                \"0\" or \"1\"."
    echo "          shadow:              \"0\" or \"1\"."
    exit 1
fi

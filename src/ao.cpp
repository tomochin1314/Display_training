...
void ao_t::init_shader()
{
    ...
    dbs_ao_shader= new shader_t("shaders/vs_depth_encoded_ao.glsl",
                                      "shaders/ps_depth_encoded_ao.glsl");
    ao_shader    = new shader_t("shaders/vs_ao.glsl", "shaders/ps_ao.glsl");

    normalmap_shader = new shader_t("shaders/vs_normalmap.glsl", "shaders/ps_normalmap.glsl");

    normalmap_sign_shader = new shader_t("shaders/vs_normalmap_sign.glsl", "shaders/ps_normalmap_sign.glsl");

    combine_shader = new shader_t("shaders/vs_combine.glsl", "shaders/ps_combine.glsl");

    if(depth_based_shadow)
        p_ao_shader = dbs_ao_shader;
    else p_ao_shader = ao_shader;
...
void ao_t::gen_reloc_depth_tex(GLuint w, GLuint h)
{
...
void ao_t::generateShadowFBO()
{
    ...
    //
    // create normal map texture, must be float type
    //
    glGenTextures(1, &normal_map_tex_id);
    glBindTexture(GL_TEXTURE_2D, normal_map_tex_id);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA16, tr->m_width, tr->m_height, 0, 
                  GL_RGBA, GL_FLOAT, 0);
    glBindTexture(GL_TEXTURE_2D, 0);
    
    glGenTextures(1, &normal_map_sign_tex_id);
    glBindTexture(GL_TEXTURE_2D, normal_map_sign_tex_id);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    ...
    // ao map texture, used to store the ap result
    glGenTextures(1, &ao_map_tex_id);
    glBindTexture(GL_TEXTURE_2D, ao_map_tex_id);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
    glTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
...
void ao_t::setTextureMatrix(GLuint tex_unit_id)
{
...
// the first pass
//
void ao_t::render_reloc_tube_depth(float scale, GLuint depth_tex_id)
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER,0);	
    glPopAttrib();
    printf("reloc depth:");print_error();    
}
...
// the second pass
//
void ao_t::render_shadow_mask_tex()
{
    // if not push all attrib here, 
    // just push enable bits and viewport bits, will be somehing wrong
    // I don't know why
    ...
    printf("shadow mask: ");print_error();    
}

void ao_t::render_normalmap()
{
    glPushAttrib(GL_ALL_ATTRIB_BITS);
       
    normalmap_shader->on();

    normalmap_shader->set_uniform3f("camera_pos", eye[0], eye[1], eye[2]);
    normalmap_shader->set_uniform3f("camera_dir", dir[0], dir[1], dir[2]); 
    normalmap_shader->set_uniform1f("depth_far", tr->m_far);
    normalmap_shader->set_uniform1f("depth_near", tr->m_near);
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D, normal_map_tex_id, 0);
    glDrawBuffer(GL_COLOR_ATTACHMENT0);
...
void ao_t::render_normalmap_sign()
{
    glPushAttrib(GL_ALL_ATTRIB_BITS);
       
    normalmap_sign_shader->on();

    glBindFramebuffer(GL_FRAMEBUFFER,fbo_id);	//Rendering offscreen
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);

    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D, normal_map_sign_tex_id, 0);
    glDrawBuffer(GL_COLOR_ATTACHMENT0);
    glClearColor(0.0, 0.0, 0.0, 1.0);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);    
    glViewport(0, 0, tr->m_width, tr->m_height);
...
void ao_t::render_ao_tex()
{
    ...
    //glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glBindFramebuffer(GL_FRAMEBUFFER, fbo_id);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D, ao_map_tex_id, 0);
    glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE); 

    // normal map
    glActiveTextureARB(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, normal_map_tex_id);
    p_ao_shader->set_uniform1i("normalMap", 0);

    // normal's sign in previous normal map
    glActiveTextureARB(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, normal_map_sign_tex_id);
    p_ao_shader->set_uniform1i("normalMapSign", 0);

    // random normals map
    glActiveTextureARB(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, rnm_tex_id);
    p_ao_shader->set_uniform1i("rnm", 1);
    ...
    glBegin(GL_QUADS);
    glTexCoord2d(0,0);glVertex3f(-tr->m_width/2,-tr->m_height/2,0);
    glTexCoord2d(1,0);glVertex3f(tr->m_width/2,-tr->m_width/2,0);
    glTexCoord2d(1,1);glVertex3f(tr->m_width/2,tr->m_height/2,0);
...
void ao_t::render_ao()
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER,0);	//Rendering offscreen
    glEnable(GL_DEPTH_TEST);
    glDepthFunc(GL_LESS);
    ...
    if(depth_based_shadow)
    {
        ...
        glBindTexture(GL_TEXTURE_2D,shadow_mask_tex_id);
        combine_shader->set_uniform1i("shadow_mask", 6);
    }
    ...
    glBindTexture(GL_TEXTURE_2D, ao_map_tex_id);
    combine_shader->set_uniform1i("ao_map", 7);
    ...
    glPopAttrib();
    printf("ao :");print_error();    
}

void ao_t::render_halo()
{
    glPushAttrib(GL_ALL_ATTRIB_BITS);
    for(GLuint i = 0; i < lights.size(); ++i)
        lights[i].off();
...
void ao_t::render_tex(GLuint tex_id)
{
    glPushAttrib(GL_ALL_ATTRIB_BITS);
    glPushClientAttrib(GL_ALL_CLIENT_ATTRIB_BITS);
...
void ao_t::render() 
{
    ...
    render_normalmap();
    render_normalmap_sign();
    render_ao_tex();
    render_ao();

    //render_tex(normal_map_tex_id);
    //render_tex(normal_map_sign_tex_id);
    //render_tex(ao_map_tex_id);
...
/*
   Read a line
   */
int ao_t::ReadLine(FILE *fptr,char *s,int lmax)
{
    int i=0,c;
...
/*
   ...
   */
BITMAP4 *ao_t::read_ppm(const char *fname,GLfloat *c,int trans,int *w,int *h,int tmode)
{
    int r,g,b;
    int i;
...
void ao_t::write_ppm(const BITMAP4 *const map, const int w, const int h)
{
...
void ao_t::read_tonal_art_maps()
{

    GLfloat white[] = { 1.0, 1.0, 1.0, 1.0 };
    rnm_pic = read_ppm("pictures/noise.ppm",white,'o',&twidth,&theight,2);
    glGenTextures(1,&rnm_tex_id);
    glBindTexture(GL_TEXTURE_2D, rnm_tex_id);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,twidth,theight,0,GL_RGBA,GL_UNSIGNED_BYTE,rnm_pic);
    gluBuild2DMipmaps(GL_TEXTURE_2D,GL_RGBA,twidth,theight,GL_RGBA,GL_UNSIGNED_BYTE,rnm_pic);
...
void ao_t::calc_shadow_len_threshold()
{

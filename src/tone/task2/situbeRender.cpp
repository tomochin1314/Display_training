...
void CSitubeRender::buildTubefromLine(unsigned long lineIdx) 
{
	...
	for (unsigned long idx1 = 0, idx2 = 1; idx1 < szPts; idx1++) {
		 ...
		 */
        GLfloat v_step = TEXCOORD_V_STEP;  // control the texcoord generation, this MACRO is defined in "tone.h" 
...
void CSitubeRender::glInit(void) 
{
    ...
    tone->set_halo(false);  // these two settings must before init
    tone->set_depth_based_shadow(false);
    tone->init_tone();
}
